...
試合中にログアウトしたプレイヤーはチームから外れず、一覧では `Logout` と表示されます。車両の HP と所属は維持され、再ログイン時には SteamID64 を使って新しい peer_id へ自動的に引き継がれます。ログアウト中の車両が破壊された場合は通常どおり死亡扱いです。ただし、接続中の生存者がおらず `Logout` のみになったチームは敗北扱いです。試合外でのログアウトは従来どおりチームから離脱します。
...
- `?mm set heavy_damage_mul [倍率]`<br>
  ...
  前提:OnVehicleDamagedでは小さいダメージイベントが数百回発生します。
　BCの中心部は3ダメージが入るため、ダメージが3以上の場合にこの係数を適用します。
　例：倍率10では、2300ダメージ程度の車両が約3100ダメージになります。
