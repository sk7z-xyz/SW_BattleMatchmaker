...
参加人数が奇数の場合、2チーム戦ではどちらか一方が 1 人多くなり、人数が多いチームが有利になりやすくなります。
以前の `?mm sh2`（`?mm shuffle2`）は余り人数を RED 側へ固定配分していたため、たとえば 5 人なら RED 3 人、BLUE 2 人となり、RED が人数面で有利でした。
現在の `?mm sh2` は余り人数をランダムなチームへ配分するため、奇数人数による人数差は残っても、RED が固定で有利になることはありません。過去のチーム編成も考慮して偏りを抑えます。なお、単純シャッフルの `?mm sh`（`?mm shuffle`）は現在も RED から順に配分します。`?mm sh` は履歴を考慮しないため、前回と同じチーム・同じ味方が続く可能性を抑えられず、プレイヤーの入れ替わりが少なくなる場合があります。
...
参加者をランダムに取り出し、RED、BLUE、PINK、YLW の順へ循環配置します。人数差は最大 1 人です。履歴を参照しないため、前回と同じチーム・同じ味方が続く可能性があります。Ready は全解除され、既存カウントダウンは停止します。

#### `shuffle2`

- チーム人数差を最大 1 人にします。
- 奇数人数の余りはランダムなチームへ配分します。以前のように RED へ固定配分しないため、RED 固定の人数有利はありません。
